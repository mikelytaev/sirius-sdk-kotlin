...
class TestWallet {
    ...
    fun testCryptoSign() {
        LibsodiumInitializer.initializeWithCallback {
            ...
            val isOk2: Boolean =
                walletVerifier?.crypto?.cryptoVerify(keySigner2, messageBytes, brokenSignature) ?: false
            assertFalse(isOk2)
            agent1.close()
            agent2.close()
        }
    ...
    fun testRecordSearch() {
        LibsodiumInitializer.initializeWithCallback {
            ...
            val (searchList, second) = agent1.getWalleti()?.nonSecrets?.walletSearch("type", query.toString(), opts)
                ?: Pair(
                    listOf(), 0
                )
            ...
            val (searchList2, second1) = agent1.getWalleti()?.nonSecrets?.walletSearch(
                "type",
                queryNew.toString(),
                opts
            ) ?: Pair(
            ...
            val (first, second2) = agent1.getWalleti()?.nonSecrets?.walletSearch(
                "type",
                queryNew.toString(),
                opts,
                1000
            ...
            val (_, second3) = agent1.getWalleti()?.nonSecrets?.walletSearch("type", queryNew1.toString(), opts, 1000)
                ?: Pair(
                    listOf(), 0
                )
