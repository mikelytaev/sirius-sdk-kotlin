...
class PairwiseMobile(wallet: Wallet, nonSecretsMobile: NonSecretsMobile) : AbstractPairwise() {
    var wallet: Wallet
    var timeoutSec = 60
    val STORAGE_TYPE = "pairwise"
    val CONST_VALUE = "pairwise"
    val DEFAULT_FETCH_LIMIT = 1000
    var nonSecretsMobile: NonSecretsMobile
    fun isPairwiseExist(theirDid: String?): Boolean {
        try {
            return Pairwise.isPairwiseExists(wallet, theirDid).get(timeoutSec, java.util.concurrent.TimeUnit.SECONDS)
    ...
    fun createPairwise(theirDid: String?, myDid: String?, metadata: JSONObject, tags: JSONObject?) {
        try {
    ...
    fun listPairwise(): List<Any>? {
        try {
            val listPairwise: String =
                Pairwise.listPairwise(wallet).get(timeoutSec, java.util.concurrent.TimeUnit.SECONDS)
    ...
    fun getPairwise(thierDid: String?): String? {
        try {
            val pairwiseInfoJson: String =
                Pairwise.getPairwise(wallet, thierDid).get(timeoutSec, java.util.concurrent.TimeUnit.SECONDS)
    ...
    fun setPairwiseMetadata(theirDid: String?, metadata: JSONObject, tags: JSONObject?) {
        try {
            updateWalletRecordValueTagsSafely(STORAGE_TYPE, theirDid, tags)
    ...
    fun search(tags: JSONObject, limit: Int?): Pair<List<String>, Int> {
        var limit = limit
