...
actual  class CryptoMobile actual constructor(val wallet: LocalWallet) : AbstractCrypto() {
    ...
    override fun packMessage(message: Any?, recipentVerkeys: List<String>?, senderVerkey: String?): ByteArray? {
     ...
     //   val byteMessage: ByteArray = StringUtils.stringToBytes(messageString, StringUtils.CODEC.US_ASCII)
        val byteMessage: ByteArray = messageString.toByteArray(java.nio.charset.StandardCharsets.US_ASCII)
        try {
    ...
    override fun unpackMessage(jwe: ByteArray?): String? {
        try {
            val bytes = Crypto.unpackMessage(wallet, jwe).get(timeoutSec.toLong(), java.util.concurrent.TimeUnit.SECONDS)
            return StringUtils.bytesToString(bytes, StringUtils.CODEC.US_ASCII)
        } catch (e: java.lang.Exception) {
            e.printStackTrace()
        }
        return null
