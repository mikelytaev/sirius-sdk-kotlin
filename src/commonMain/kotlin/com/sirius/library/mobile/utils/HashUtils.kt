...
*/
    fun generateHash(password: String): String {
        val hash256 =  Hash.sha256(StringUtils.stringToBytes(password, StringUtils.CODEC.UTF_8).toUByteArray())
        return LibsodiumUtil.toHex(hash256)

    }
    /*
    @Throws(NoSuchAlgorithmException::class, InvalidKeySpecException::class)
    fun generateHash(password: String): String {
       val hash256 =  Hash.sha256(StringUtils.stringToBytes(password, StringUtils.UTF_8).toUByteArray())
        LibsodiumUtil.toHex(hash256)
        val iterations = 1000
        val saltStr = "ABC"
        val salt = saltStr.toByteArray()
        val chars = password.toCharArray()
